...
- `genimage.sh` - Builds the installer SD card image. You only need to execute this script if you need more than a .zip package. The script produces an .img package and then its bzip2 and xz compressed versions, but this is configurable (see below).
...
To set genimage/buildroot options, create a file named `buildroot.conf`, which contains the appropriate variable settings. Supported variables are:
